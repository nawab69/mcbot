...
const msg = message.content;               // store input message
const url = "http://api.steemtools.cf/get_blog_count?url=";       // steemtools api
const api = url.concat(msg);                                    // generate api address

 Request.get(api, (error, response, body) => { if(error) { return console.dir(error);
 // Get data in JSON from API by HTTP request
  }
 
const result = JSON.parse(body);        // convert json to object
